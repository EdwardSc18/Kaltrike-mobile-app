...
import 'package:kaltrike_driver_app/src/features/driver/auth/presentation/screens/signup_screen.dart';
...
import 'package:kaltrike_driver_app/src/features/driver/auth/presentation/screens/auth_loading_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _loadSavedCredentials() async {
    try {
      ...
      final savedEmail = prefs.getString('saved_driver_email');
      final savedPassword = prefs.getString('saved_driver_password');
      final rememberMe = prefs.getBool('remember_driver_me') ?? false;
      ...
      print('Error loading saved driver credentials: $error');
    }
  }
  ...
  Future<void> _saveCredentials() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      if (_rememberMe) {
        await prefs.setString('saved_driver_email', emailTextEditingController.text.trim());
        await prefs.setString('saved_driver_password', passwordTextEditingController.text.trim());
        await prefs.setBool('remember_driver_me', true);
      } else {
        await prefs.remove('saved_driver_email');
        await prefs.remove('saved_driver_password');
        await prefs.setBool('remember_driver_me', false);
      }
    } catch (error) {
      print('Error saving driver credentials: $error');
    }
  }
  ...
  Future<void> _clearCredentials() async {
    try {
      ...
      print('Error clearing driver credentials: $error');
    }
  }
  ...
  String _getUserFriendlyErrorMessage(FirebaseAuthException error) {
    switch (error.code) {
      ...
      case 'user-disabled':
        return "This driver account has been disabled. Please contact support.";
      case 'user-not-found':
        return "No driver account found with this email. Please check your email or sign up.";
      case 'wrong-password':
  ...
  // Firebase login with enhanced error handling
  Future<void> loginDriverNow() async {
    ...
    showDialog(
      ...
      builder: (BuildContext c) {
        return const ProgressDialog(message: "Signing you in...");
      },
    );
  ...
  Future<void> _resetPassword() async {
    ...
    showDialog(
      ...
      barrierDismissible: false,
      builder: (BuildContext c) {
        return ProgressDialog(message: "Sending reset instructions...");
      },
    );
    ...
    try {
      ...
      if (error.code == 'user-not-found') {
        _showErrorToast("No driver account found with this email address.");
      } else {
